defmodule Hangman.Counter do
  ...
  def add_letters(%Counter{map: map} = counter, codepoints) do
    ...
    map_updated =
      Enum.reduce(codepoints, map, fn head, acc -> Map.update(acc, head, 1, &(&1 + 1)) end)
  ...
  def add_words(%Counter{} = counter, words)
    ...
    counter =
      Enum.reduce(grouped_letter_counts, counter, fn {codepoint_value, value}, acc ->
  ...
  def add_words(%Counter{} = counter, words_stream, exclusion_set) do
    # the acc is the first param, so switch
    counter =
      words_stream
      |> Stream.chunk(@chunk_words_size, @chunk_words_size, [])
      |> Enum.reduce(counter, &add_words(&2, &1))
