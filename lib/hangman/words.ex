defmodule Hangman.Words do
  ...
  def add(%Words{binary_stream: binary_stream} = words, {binary_list, count} = _value)
    ...
    %{words | binary_stream: new_stream, count: words.count + count}
  end
  ...
  def filter(%Words{key: key} = words, regex_filter_key) do
    filtered = words |> stream |> Stream.filter(&regex_match?(&1, regex_filter_key))
  ...
  defp create(length_key, %Stream{} = words)
    ...
    fn_split_into_containers = fn {_word, index} ->
