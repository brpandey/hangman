defmodule Hangman.Game.Server do
  ...
  @callback handle_call({Guess.t(), Player.key(), Round.key()}, tuple, Registry.t()) :: tuple
  def handle_call({guess = {:guess_letter, _letter}, player_key, round_key}, _from, state) do
  ...
  @callback handle_call({Guess.t(), Player.key(), Round.key()}, tuple, Registry.t()) :: tuple
  def handle_call({guess = {:guess_word, _word}, player_key, round_key}, _from, state) do
  ...
  def handle_info({:DOWN, ref, :process, pid, :normal}, state) do
    _ =
      Logger.debug(
        "In Game.Server handle info, received :DOWN normal msg, self: #{inspect(self())}"
      )
  ...
  def handle_info({:DOWN, ref, :process, pid, reason}, state) do
    _ =
      Logger.debug(
        "In Game.Server handle info, received :DOWN msg, self: #{inspect(self())}, reason: #{
          inspect(reason)
        }"
      )
  ...
  def terminate(reason, _state) do
    _ =
      Logger.debug(
        "Terminating Hangman Game Server reason: #{inspect(reason)}, #{inspect(self())}"
      )
