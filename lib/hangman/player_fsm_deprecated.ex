defmodule Hangman.Player.FSM do
  ...
  def eager_socrates({:guess_letter, letter}, _from, {player, pid}) do

    {player, reply} = 
      Player.guess(player, :human, {:guess_letter, letter}, :game_keep_guessing)
    ...
    if next == :eager_socrates do
      {player, reply} = Player.guess(player, :human, 
                                     :choose_letters, :game_keep_guessing)
  ...
  def giddy_socrates(:guess_last_word, _from, {player, pid}) do
    ...
    {player, reply} = Player.guess(player, :human, :guess_last_word, :game_last_word)
  ...
  def intrigued_wall_e(:game_keep_guessing, _from, {player, pid}) do
    ...
    {player, reply} = Player.guess(player, :robot, :game_keep_guessing)
    {status_code, _} = reply
  ...
  def spellbound_wall_e(:game_keep_guessing, {player, echo_pid}) do
    ...
    {player, {status_code, _}} = 
      Player.guess(player, :robot, :game_keep_guessing)
