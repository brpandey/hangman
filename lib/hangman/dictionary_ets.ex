defmodule Hangman.Dictionary.ETS do
  ...
  @spec put(:words | :random | :counter, {pos_integer, [String.t()] | Counter.t()}) ::
          :ok | no_return
  ...
  def put(:random, {key, list})
    ...
    cond do
      key >= @min_random_word_length and key <= @max_random_word_length ->
        ...
        rand =
          for _x <- 1..@random_words_per_container do
            Enum.random(list)
          end
  ...
  def put(:counter, {key, %Counter{} = counter})
    ...
    case valid_key?(key) do
      true ->
        _ =
          Logger.debug(
            "Dictionary.ETS.put, table is #{@ets_table_name} length is #{key}, counter is #{
              inspect(counter)
            }"
          )
