defmodule Hangman.Player.Worker.Test do
  ...
  setup_all do
    ...
    map = %{
      ...
      :cases => %{
        :robot => [name: {"wall_e_test", 1}, type: :robot, display: true, secrets: ["cumulate"]],
        :human => [
          name: "socrates_test",
          type: :human,
          display: true,
  ...
  setup context do
    ...
    map = Map.put(map, :current_player_id, name)
  ...
  test "synchronous robot player over 1 game", context do
    ...
    id = context[:params] |> Map.get(:current_player_id)

    # :sys.trace(ppid, true)

    {:begin, _reply} = id |> Player.Worker.proceed()
    {:action, reply} = id |> Player.Worker.proceed()
  ...
  test "synchronous human player over 2 games", context do
    ...
    {:begin, reply} = id |> Player.Worker.proceed()

    IO.puts("\nGame 1a: #{inspect(reply)}")

    {:setup, setup} = id |> Player.Worker.proceed()

    IO.puts("\nGame 1b: #{inspect(setup)}")

    {:action, reply} = id |> Player.Worker.guess("e")
    ...
    {:action, reply} = id |> Player.Worker.guess("a")
    ...
    {:action, reply} = id |> Player.Worker.guess("t")
    ...
    {:action, reply} = id |> Player.Worker.guess("o")
    ...
    {:action, reply} = id |> Player.Worker.guess("i")
    ...
    {:action, reply} = id |> Player.Worker.guess("l")
    ...
    assert [
             ...
             status:
               {:guess_word, "cumulate",
                "Player socrates_test, Round 9, C-M-LATE; score=8; status=KEEP_GUESSING.\nLast word left: cumulate"}
           ] = setup

    {:action, reply} = id |> Player.Worker.guess("cumulate")
    ...
    # transition stop
    IO.puts("\nGame 2a: #{inspect(reply)}")

    {:begin, reply} = id |> Player.Worker.proceed()

    # begin
    IO.puts("\nGame 2b: #{inspect(reply)}")

    {:setup, setup} = id |> Player.Worker.proceed()

    # setup
    IO.puts("\nGame 2c: #{inspect(setup)}")
    ...
    {:action, reply} = id |> Player.Worker.guess("s")
    ...
    {:action, reply} = id |> Player.Worker.guess("r")
    ...
    {:action, reply} = id |> Player.Worker.guess("d")
    ...
    {:action, reply} = id |> Player.Worker.guess("avocado")
