defmodule Hangman.Game.UnitTest do
  ...
  describe "game with single secret" do    
    ...
    test "secret length should be valid", %{game: game} do
      assert String.length("genealogy") == 
        game |> Game.secret_length
    end
    ...
    test "game status when start with no guesses", %{game: game} do
      ...
      assert %{id: "orange", code: :guessing, 
               text: "---------; score=0; status=KEEP_GUESSING"} == feedback
    end
    ...
    test "game status after 1st incorrect guess", %{game: game} do
      ...
      assert %{id: "orange", code: :guessing, text: "---------; score=1; status=KEEP_GUESSING"} == feedback
    end
    ...
    test "game status after 2nd incorrect guess", %{game: game} do
      ...
      assert  %{id: "orange", code: :guessing, 
                text: "---------; score=2; status=KEEP_GUESSING"} == feedback
    end
    ...
    test "game status after 1st correct guess", %{game: game} do
      ...
      assert %{id: "orange", code: :guessing, 
               text: "G------G-; score=1; status=KEEP_GUESSING"} == feedback
    end
    ...
    test "game status after 2nd correct guess", %{game: game} do
      ...
      assert  %{id: "orange", code: :guessing, 
                text: "G----L-G-; score=2; status=KEEP_GUESSING"} == feedback
    end
    ...
    test "guess next error if game still being played", %{game: game} do
      ...
      assert catch_error(Game.next(game)) == 
        %HangmanError{message: "not supported - calling of next when single game is not over"}
    end
  ...
  describe "game with multiple secrets" do
    ...
    test "guess next return next game when first game finished", %{game: game} do
      ...
      assert %{id: "orange", code: :start, 
               text: 'GAME_START', previous: %{code: :won, id: "orange", text: "GENEALOGY; score=1; status=GAME_WON"}}
      == feedback

      {_game, result} = Game.guess(game, {:guess_letter, "p"})

      assert %{code: :guessing, id: "orange", pattern: "P--------", 
               result: :correct_letter, 
               text: "P--------; score=1; status=KEEP_GUESSING"} == result
    end
    ...
    test "guess next return game over when all games finished", %{game: game} do
      ...
      assert %{code: :guessing, id: "orange", pattern: "P-O--O---", 
               result: :correct_letter, 
               text: "P-O--O---; score=2; status=KEEP_GUESSING"} == result

      {game, result} = Game.guess(game, {:guess_letter, "i"})

      assert %{code: :guessing, id: "orange", 
               result: :correct_letter, pattern: "P-O-IO-I-", 
               text: "P-O-IO-I-; score=3; status=KEEP_GUESSING"} == result

      {game, result} = Game.guess(game, {:guess_word, "probiotic"})


      assert %{id: "orange", code: :won, 
               text: "PROBIOTIC; score=3; status=GAME_WON", 
               pattern: "PROBIOTIC", result: :correct_word} == result
