defmodule Hangman.Player.FSM.Test do
  ...
  setup_all do
    ...
    map = %{
      :fsm_args => nil,
      :cases => %{
        :robot => [name: "wall_e_test", type: :robot, display: true, secrets: ["immaculate"]],
        :human => [
          name: "socrates_test",
          type: :human,
          display: true,
  ...
  test "test human fsm play", context do
      ...
      response ==
        {:setup,
         [
           ...
           status:
             {:guess_word, "tulip",
              "Player socrates_test, Round 6, -U-IP; score=5; status=KEEP_GUESSING.\nLast word left: tulip"}
         ]}
    )
    ...
    assert(
      response ==
        {:exit, "Game Over! Average Score: 6.5, Games: 2, Scores:  (TULIP: 5) (DAISY: 8)"}
    )
  end
  ...
  test "test robot fsm play", context do
    ...
    assert(
      response == {:transit, "Game Over! Average Score: 4.0, Games: 1, Scores:  (IMMACULATE: 4)"}
    )
    ...
    assert(
      response == {:exit, "Game Over! Average Score: 4.0, Games: 1, Scores:  (IMMACULATE: 4)"}
    )
