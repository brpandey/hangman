...
defmodule Hangman.Counter.Test do
  ...
  describe "counter create" do
    ...
    test "create with map" do
      ...
      assert [{"b", 15}, {"c", 29}, {"d", 12}, {"f", 6}, {"g", 9}, {"h", 9}, {"i", 43},
              {"j", 3}, {"k", 2}, {"l", 35}, {"m", 17}, {"n", 27}, {"o", 42}, {"p", 13},
              {"r", 24}, {"s", 20}, {"u", 40}, {"v", 6}, {"w", 1}, {"x", 1}, {"y", 2},
              {"z", 1}] = map |> Counter.new |> Counter.items     
    end
  ...
  describe "counter read operations" do
    ...
    test "most common non-empty counter" do
      ...
      assert ["-", "a", "x"] = 
        "a---ax---a" |> Counter.new |> Counter.most_common_key(5)

      assert [{"-", 9}, {"a", 3}] = 
        "-x--a---a---a" |> Counter.new |> Counter.most_common(2)

      assert ["-", "a"] = 
        "-x--a---a---a" |> Counter.new |> Counter.most_common_key(2)

      assert [{"-", 10}] = 
        "---a---a-x--a-" |> Counter.new |> Counter.most_common(1)

      assert ["-"] = 
        "---a---a-x--a-" |> Counter.new |> Counter.most_common_key(1)
    end
  ...
  describe "counter add operations" do
    ...
    test "add non-unique letters from string and tally" do
      ...
      assert [{"i", 4}, {"s", 4}, {"p", 2}, {"m", 1}] =
        tally |> Counter.most_common(5)

      assert [{"i", 4}, {"m", 1}, {"p", 2}, {"s", 4}] = 
        tally |> Counter.items
    end
    ...
    test "add words list" do
      ...
      assert [{"a", 9}, {"b", 3}, {"c", 24}, {"d", 6}, {"e", 18}, {"f", 3}, {"g", 2},
              {"h", 1}, {"i", 8}, {"l", 9}, {"m", 3}, {"n", 21}, {"o", 24}, {"p", 2},
              {"r", 16}, {"s", 16}, {"t", 20}, {"u", 17}, {"w", 3}, {"y", 1}] = 
        tally |> Counter.items
    ...
    test "add words list stream with exclusion" do
      ...
      list = ["cotoneaster","cotransduce","cotransfers","cotransport","cottonmouth","cottonseeds","cottontails","cottonweeds","cottonwoods","cotylosaurs","coulometers","coulometric","councillors","counselings","counselling","counsellors","countenance","counteracts","counterbade","counterbids","counterblow","countercoup","counterfeit","counterfire"]
      ...
      assert [{"b", 3}, {"c", 24}, {"d", 6}, {"e", 18}, {"f", 3}, {"g", 2}, {"h", 1},
              {"i", 8}, {"l", 9}, {"m", 3}, {"n", 21}, {"o", 24}, {"r", 16},
              {"s", 16}, {"t", 20}, {"u", 17}, {"w", 3}, {"y", 1}] = 
        tally |> Counter.items
  ...
  describe "general CRUD behavior" do
    ...
    test "crud mixed together detailed" do
      ...
      tally = tally |> Counter.delete(["i", "o", "u"]) 
      ...
      assert [{"A", 2}, {"C", 1}, {"E", 6}, {"K", 1}, {"O", 3}, {"V", 1},
            {"a", 9}, {"b", 18}, {"c", 53}, {"d", 18}, {"e", 18}, {"f", 9},
            {"g", 11}, {"h", 10}, {"i", 9}, {"j", 3}, {"k", 2}, {"l", 44},
            {"m", 21}, {"n", 48}, {"o", 24}, {"p", 16}, {"r", 40}, {"s", 37},
            {"t", 20}, {"u", 17}, {"v", 6}, {"w", 4}, {"x", 1}, {"y", 3},
            {"z", 1}] =
        tally |> Counter.items
