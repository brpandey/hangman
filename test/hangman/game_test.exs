defmodule Hangman.Game.Test do
  ...
  test "fred - game w/o server" do
      ...
      assert %{id: "fred", code: :guessing, pattern: "-XO---",
      ...
      game_text = "#Game<[id: \"fred\", state: :guessing, current_game_index: 0, secret: \"EXOTIC\", pattern: \"EXOT--\", score: 0, secrets: [\"EXOTIC\"], patterns: [], scores: [], max_wrong_guesses: 5, guessed_letters: [correct: [\"E\", \"O\", \"T\", \"X\"], incorrect: [\"S\", \"U\"]], guessed_words: [incorrect: [\"EXOTLY\"]]]>"

      assert game_text == "#{inspect game}"

      assert %{id: "fred", result: :incorrect_word, code: :guessing, 
               pattern: "EXOT--", text: "EXOT--; score=7; status=KEEP_GUESSING"}
               = result

      {game, result} = Game.guess(game, {:guess_word, "exotic"})
      
      assert %{id: "fred", result: :correct_word, code: :won, 
