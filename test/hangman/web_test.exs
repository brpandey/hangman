defmodule Hangman.Web.Test do
  ...
  test "cowboy http server with single secret kiwi" do
    body2 =
      "(H) ----; score=1; status=KEEP_GUESSING (H) ----; score=2; status=KEEP_GUESSING (H) ----; score=3; status=KEEP_GUESSING (H) -I-I; score=4; status=KEEP_GUESSING (H) -I-I; score=5; status=KEEP_GUESSING (H) -I-I; score=6; status=KEEP_GUESSING (H) -I-I; score=25; status=GAME_LOST (H) Game Over! Average Score: 25.0, Games: 1, Scores:  (KIWI: 25) "
  ...
  test "cowboy http server with 40 random secrets print out" do
    {:ok, response = %HTTPoison.Response{}} =
      HTTPoison.get(
        "http://127.0.0.1:3737/hangman?name=melvin&random=40",
        [],
    ...
    IO.puts(
      "HTTPoison.get http://127.0.0.1:3737/hangman?name=melvin&random=40 gives: #{
  ...
  test "cowboy http server with 15 random secrets" do
    ...
    IO.puts(
      "HTTPoison.get http://127.0.0.1:3737/hangman?name=orange&random=15 gives: #{
        inspect(response.body)
      }\n\n"
    )
  end
  ...
  test "cowboy http server with secrets list entries talented and hermetic" do
    ...
    {:ok, response = %HTTPoison.Response{}} =
      HTTPoison.get(
        "http://127.0.0.1:3737/hangman?name=carmen&secret[]=talented&secret[]=hermetic"
      )
  ...
  test "cowboy http server with a name token, secret and random value, secret takes precedence" do
    ...
    assert response.body ==
             "(H) ----------E; score=1; status=KEEP_GUESSING (H) ----------E; score=2; status=KEEP_GUESSING (H) ----------E; score=3; status=KEEP_GUESSING (H) I-------I-E; score=4; status=KEEP_GUESSING (H) INSTRUCTIVE; score=4; status=GAME_WON (H) Game Over! Average Score: 4.0, Games: 1, Scores:  (INSTRUCTIVE: 4) "
  end

  test "cowboy http server with secrets list" do
    {:ok, response = %HTTPoison.Response{}} =
      HTTPoison.get(
        "http://127.0.0.1:3737/hangman?name=gustav&secret[]=cumulate&secret[]=avocado&secret[]=eruptive"
      )
  ...
  test "cowboy http server with 200 random secrets print out" do
    {:ok, response = %HTTPoison.Response{}} =
      HTTPoison.get(
        "http://127.0.0.1:3737/hangman?name=melvin&random=200",
        [],
        recv_timeout: :infinity
      )
    ...
    IO.puts(
      "HTTPoison.get http://127.0.0.1:3737/hangman?name=melvin&random=200 gives: #{
        inspect(response.body)
      }\n\n"
    )
  end
