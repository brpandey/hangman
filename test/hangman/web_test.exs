defmodule Hangman.Web.Test do
  ...
  test "cowboy http server" do
    ...
    {:ok, response2 = %HTTPoison.Response{}} =
      HTTPoison.get("http://127.0.0.1:3737/hangman?name=julio&secret=kiwi")
    
    assert response2.body == body2
    
    {:ok, response3 = %HTTPoison.Response{}} =
      HTTPoison.get("http://127.0.0.1:3737/hangman?name=julio&random=2")
    
    IO.puts "HTTPoison.get http://127.0.0.1:3737/hangman?name=julio&random=2 gives: #{inspect response3.body}\n\n"
  end
end
