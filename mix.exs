...
#      mod: {Hangman.Application, [type: :big, ingestion: true]}
  ...
  defp deps do
    [
      ...
      {:plug, "~> 1.3"},  # for hangman web
      {:httpoison, "~> 0.11.0"}, # for hangman web
      {:exrm, "~> 1.0"}, # for mix release
      {:fsm, "~> 0.3.0"}, # for state machine handling
      {:exactor, "~> 2.2.0", warn_missing: false}, # for simple GenServers
      {:gen_stage, "~> 0.10"}, # exp module for distinct producers and consumers
      {:eqc_ex, "~> 1.4"}, # for quick check property tests
      {:dialyxir, "~> 0.4.3", only: [:dev]} # for erlang dialyzer type checking
    ] 
  end
end
